...
def process_rows(row):
    ...
    Pack Type: {source_packtype}, Pack Size: {llm_prompt_qty} {llm_prompt_uom}, Product Category Code : {source_catcode}, \
    ...
    if master_filtered_1.shape[0] < 1:
        ...
        master_filtered_1 = filter_similar_products(llm_prompt,master_filtered_for_llm)
        if debug:
...
def process_csv(filename,startindex,endindex):
    ...
    df_results.to_csv(f"D:/Rohit/ORG_SKUR/processed_result_chunk_3/filtered_results_llm_{filename}", index=False)
